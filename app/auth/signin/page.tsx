"use client";

import { signIn } from "next-auth/react";
import { markAuthValid } from "@/components/AuthGuard";
import { AnvilLogo } from "@/components/ClaudeLogo";

export default function SignInPage() {
  return (
    <div className="min-h-[100dvh] flex items-center justify-center px-4">
      <div className="w-full max-w-sm text-center space-y-8">
        <div className="space-y-3">
          <div className="flex justify-center">
            <AnvilLogo size={48} className="text-accent" />
          </div>
          <div className="flex items-center justify-center gap-2">
            <h1 className="text-2xl font-bold">Code Anvil Mobile</h1>
            <span
              className="text-[10px] font-mono font-semibold tracking-wide rounded-full px-2 py-0.5"
              style={{
                color: "#fff",
                backgroundColor: "rgba(34, 197, 94, 0.2)",
                border: "1px solid rgba(34, 197, 94, 0.4)",
                boxShadow: "0 0 8px rgba(34, 197, 94, 0.3), 0 0 16px rgba(34, 197, 94, 0.15)",
              }}
            >
              v1.2.0
            </span>
          </div>
          <p className="text-sm text-muted">
            Remote Claude Code from your phone
          </p>
        </div>

        <button
          onClick={() => {
            markAuthValid();
            signIn("google", { callbackUrl: "/browse" });
          }}
          className="mx-auto flex items-center justify-center gap-3 rounded-full px-8 py-3 font-medium transition-all duration-200 border border-border/80 bg-card/60 text-foreground hover:bg-card hover:border-accent/50 hover:shadow-[0_0_20px_rgba(59,130,246,0.15)] active:scale-[0.98]"
        >
          <svg className="w-5 h-5 shrink-0" viewBox="0 0 24 24">
            <path
              d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
              fill="#4285F4"
            />
            <path
              d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
              fill="#34A853"
            />
            <path
              d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
              fill="#FBBC05"
            />
            <path
              d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
              fill="#EA4335"
            />
          </svg>
          Sign in with Google
        </button>

        <p className="text-xs text-muted/60">
          Restricted to authorized users only
        </p>
      </div>
    </div>
  );
}
